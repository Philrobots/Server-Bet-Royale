...
class GiftResource(Resource):
    ...
    def __init__(self, gift_service: GiftService, gift_response_schema: GiftResponseSchema) -> None:
    ...
    def get(self):
        ...
        return Response(self.gift_response_schema.dump(gift), status=200)
    
    def receive_gift(self):
        auth_token = request.headers.get("Authorization", "")
        user_id = DomainId(self.token_decoder.decode_auth_token(auth_token))
        gift = self.gift_service.receive_gift(user_id)
        return Response(self.gift_response_schema.dump(gift), status=200)
